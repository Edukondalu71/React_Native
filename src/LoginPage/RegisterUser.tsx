...
import { View, Text, TextInput, TouchableOpacity, Image, Keyboard, Animated, ActivityIndicator, Alert } from 'react-native';
...
import { handleEmail, handleMobile, handlePassword } from './Utils';
...
const RegisterUser = ({ navigation }: any) => {
    ...
    const handleRegister = async () => {
        ...
        let fetchData = await regUser({ username: btoa(username), password: btoa(password), mobileNumber: btoa(mobileNumber), email:btoa(email), fcmToken: btoa(fcmToken) });
        if (fetchData.status === 200) {
            setErrorMsg(null);
            Alert.alert('user registered successfully!', 'Redirect to Login Page ?', [
                {
                    text: 'Cancel',
                    onPress: () => null,
                    style: 'cancel',
            ...
            <Animated.View style={[styles.loginBox, { height: 460 }, { transform: [{ translateY: translateValue }] }]}>
                ...
                <View style={styles.form}>
                    ...
                    <View style={styles.pswinput}>
                        ...
                        />
                        {userEmaiState === 'initial' ? null : !userEmaiState ? <Error name="error" color="red" size={18} /> : <Feather name="check-circle" color="green" size={18} />}
                    </View>
                    ...
                    <View style={styles.pswinput}>
                        ...
                        />
                        {userMobileState === 'initial' ? null : !userMobileState ? <Error name="error" color="red" size={18} /> : <Feather name="check-circle" color="green" size={18} />}
                    </View>
                    ...
                    <TouchableOpacity disabled={loader || userNameState !== true && userMobileState !== true && userPasswordState !== true && userEmaiState !== true} style={[styles.button, { opacity: loader || userNameState !== true && userMobileState !== true && userPasswordState !== true && userEmaiState !== true ? 0.5 : 1 }]} onPress={handleRegister}>
